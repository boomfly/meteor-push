...
class CordovaPush extends BasePush {
  ...
  // Системный экран уведомлений приложения: после отказа это единственный путь их включить —
  // окно разрешения ни iOS, ни Android повторно не покажут.
  async openNotificationSettings() {
    return await new Promise((resolve, reject) => {
      PushNotification.openNotificationSettings(
        () => resolve(),
        (e) => reject(new Error(e || 'openNotificationSettings failed'))
      )
    })
  }
  ...
  */
  ...
  _onRegistration(data) {
    ...
    this.console.log('device registration has been triggered with this data:', data, storage)
  ...
  _onToken(token) {
    ...
    // pending — пока сервер не ответил: на обрыве DDP держит вызов в очереди.
    ...
    Meteor.call(UPDATE_METHOD_NAME, data, (err, res) => {
      if (err) {
        ...
        this._setTokenState('failed', err.message || err)
        return
      }
  ...
  */
  ...
  _registerEvents() {
    this._push.on('registration', this._onRegistration.bind(this))
    this._push.on('notification', this._onNotification.bind(this))
    this._push.on('error', this._onError.bind(this))
    this.on('token', this._onToken.bind(this))
  }
