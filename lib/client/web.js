...
class WebPush extends EventEmitter {
  ...
  // subscribe()/unsubscribe() wait for the first config() to finish: callers invoke them right
  // after config() without awaiting it, while isSupported() and getMessaging() are still pending.
  config(configuration = {}) {
    const ready = this._configure(configuration)
    this._ready = this._ready || ready
    return ready
  }
  ...
  showNotification(title, options) {
    if (!this.messaging?.swRegistration) {
