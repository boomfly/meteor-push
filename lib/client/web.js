...
class WebPushHandle extends EventState {
        ...
        if (swSupported && !isFacebookApp()) {
          ...
          if (!noConfiguration && pushSupported) {
            ...
            window.addEventListener('load', async () => {
              ...
              WebPushEventState.emit('registration', registration)
              // Refresh all tabs after a service worker update takes place
              doTabsRefresh()
              ...
              this.messaging.onMessage(payload => {
                ...
                WebPushEventState.emit('message', payload)
  ...
  on(eventName, cb) {
    WebPushEventState.on(eventName, cb)
  }
  
  off(eventName, cb) {
    WebPushEventState.off(eventName, cb)
  }
}
