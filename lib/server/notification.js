const sendNotification = (isDebug, fcmConnections, defaults, userToken, mongoNote) => {
  ...
  const image = mongoNote.imageUrl || defaults.imageUrl
  ...
  const note = {
    android: {
      ...
      data: {
        ...
        // image: mongoNote.imageUrl || defaults.imageUrl || undefined,
        ...globalData,
        ...noteAndroidData
      },
      fcm_options: {
  ...
  if (image) {
    note.android.data.image = image
    note.apns.fcm_options.image = image
    note.webpush.notification.image = image
  }
