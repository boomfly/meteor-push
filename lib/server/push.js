...
// Снять push с каждой сессии юзера, где стоит этот токен. Позиционный $ правит один элемент за
// вызов, поэтому повторяем, пока есть совпадения (предел — с запасом больше реального числа
// сессий). Остальные поля login-токена (client, authMethod) не трогаем. Возвращает, сколько
// сессий освобождено.
const MAX_SESSIONS_PER_TOKEN = 100
export async function unsetPushToken(userId, token) {
  let removed = 0
  for (let i = 0; i < MAX_SESSIONS_PER_TOKEN; i++) {
    const modified = await Meteor.users.updateAsync(
      {_id: userId, 'services.resume.loginTokens.push.token': token},
      {$unset: {'services.resume.loginTokens.$.push': ''}}
    )
    if (!modified) break
    removed += modified
  }
...
class PushServer extends EventEmitter {
  ...
  async send(notification) {
    ...
    // Путь диплинка по приложению-получателю: токены группируются по push.app, каждая группа
    // получает data.pathname = pathnames[app]. Токен приложения без пути пуш не получает.
    if (notification.userId && notification.pathnames) {
      return this._sendByApp(notification)
    }
    ...
    if (notification.token) {
      ...
      const userTokens = this._getUserTokens(user, notification.apps)
  ...
  // Единая точка отправки в FCM (multicast или одиночный токен/топик). Тест подменяет её,
  // чтобы проверять состав сообщения без сети.
  ...
  // Путь диплинка по приложению-получателю: токены группируются по push.app, каждая группа
  // получает своё data.pathname = pathnames[app]. Токен приложения, для которого пути нет,
  // пуш не получает — это и есть «уведомление не про тебя».
  async _sendByApp(notification) {
    const user = await Meteor.users.findOneAsync(notification.userId)
    if (!user) {
      if (this._configuration.debug) {
        console.warn('User not found to send push userId:', notification.userId)
    ...
    const groups = this._getUserTokenGroups(user, notification.apps, notification.legacyApp || 'cabinet')
    const responses = []
    for (const {app, tokens} of groups) {
      const pathname = notification.pathnames[app]
      if (!pathname) continue
      const message = this._getMessage({...notification, data: {...(notification.data || {}), pathname}})
      if (tokens.length > 1) {
        message.tokens = tokens
      } else {
        message.token = tokens[0]
      }
      responses.push(await this._dispatch(message))
    }
    return responses
  }

  // Токены пользователя, сгруппированные по push.app. Легаси-токен без app (выдан до
  // появления поля) считается legacyApp — кабинетом, единственным клиентом с пушами до этого;
  // под непустой фильтр он не подходит и в группы не попадает вовсе. Фильтр тот же, что в
  // _getUserTokens.
  _getUserTokenGroups(user, apps, legacyApp) {
    const list = Array.isArray(apps) ? apps : []
    const groups = new Map()
    for (const loginToken of user.services?.resume?.loginTokens || []) {
      const push = loginToken.push
      if (!push?.token) continue
      if (list.length > 0 && !list.includes(push.app)) continue
      const app = push.app || legacyApp
      const tokens = groups.get(app) || []
      if (!tokens.includes(push.token)) tokens.push(push.token)
      groups.set(app, tokens)
    }
    return [...groups.entries()].map(([app, tokens]) => ({app, tokens}))
  }
  ...
  // Токен FCM признал протухшим: снимаем с анонимных и со всех сессий, где он стоит. Точечно
  // (unsetPushToken), а не пересборкой loginTokens — снимок затирал бы client/push соседних
  // сессий, записанные параллельно.
  async _removeToken(token) {
    ...
    await Meteor.users.find(
      ...
      await unsetPushToken(user._id, token)
    })
  }

  // Фильтр адресатов по стабильному ключу приложения (push.app: 'admin'|'cabinet'|'spaces').
  // Пустой список — без фильтра, все токены. Токен, выданный до появления push.app, поля не
  // имеет и под непустой фильтр не подходит: он перерегистрируется при следующем заходе в
  // приложение (см. methods.js), и до тех пор адресные пуши на него не идут.
  _getUserTokens(user, apps) {
    const list = Array.isArray(apps) ? apps : []
    return (user.services?.resume?.loginTokens || [])
    .filter((loginToken) => !!loginToken.push?.token)
    .filter((loginToken) => list.length === 0 || list.includes(loginToken.push?.app))
