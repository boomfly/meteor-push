...
class PushServer extends EventEmitter {
  ...
  async send(notification) {
    ...
    if (notification.token) {
      ...
      const userTokens = this._getUserTokens(user, {apps: notification.apps, appNames: notification.appNames})
  ...
  async _sendByApp(notification) {
    ...
    const filter = {apps: notification.apps, appNames: notification.appNames}
    const groups = this._getUserTokenGroups(user, filter, notification.legacyApp || 'cabinet')
    const responses = []
  ...
  // Фильтр тот же, что в _getUserTokens.
  _getUserTokenGroups(user, filter, legacyApp) {
    const apps = Array.isArray(filter?.apps) ? filter.apps : []
    const appNames = Array.isArray(filter?.appNames) ? filter.appNames : []
    const hasFilter = apps.length > 0 || appNames.length > 0
    const groups = new Map()
    for (const loginToken of user.services?.resume?.loginTokens || []) {
      const push = loginToken.push
      if (!push?.token) continue
      if (hasFilter && !apps.includes(push.app) && !appNames.includes(push.appName)) continue
  ...
  // Фильтр адресатов. `apps` — стабильный ключ приложения (push.app: 'admin'|'cabinet'|'spaces'),
  // `appNames` — легаси-фолбэк по отображаемому имени для токенов, выданных до появления push.app
  // (они перерегистрируются при следующем заходе — см. methods.js). Без фильтра — все токены.
  _getUserTokens(user, filter) {
    const apps = Array.isArray(filter?.apps) ? filter.apps : []
    const appNames = Array.isArray(filter?.appNames) ? filter.appNames : []
    const hasFilter = apps.length > 0 || appNames.length > 0
    return (user.services?.resume?.loginTokens || [])
    .filter((loginToken) => !!loginToken.push?.token)
    .filter((loginToken) => !hasFilter || apps.includes(loginToken.push?.app) || appNames.includes(loginToken.push?.appName))
