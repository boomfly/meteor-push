...
class PushServer extends EventEmitter {
  ...
  async send(notification) {
    ...
    if (notification.token) {
      ...
      const userTokens = this._getUserTokens(user, notification.appNames)
  ...
  // appNames (optional): array of app names to restrict delivery to (e.g. only
  // admin-app tokens for support-agent pushes). When omitted/empty, all of the
  // user's tokens are used (previous behaviour).
  _getUserTokens(user, appNames) {
    const hasFilter = Array.isArray(appNames) && appNames.length > 0
    return (user.services?.resume?.loginTokens || [])
    .filter((loginToken) => !!loginToken.push?.token)
    .filter((loginToken) => !hasFilter || appNames.includes(loginToken.push?.appName))
